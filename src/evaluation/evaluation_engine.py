...
from ..core.bigcodebench_integration import CustomBigCodeBenchRunner, BigCodeBenchResult, BigCodeBenchConfig
...
class EvaluationConfig:
    ...
    # Task selection
    task_types: List[TaskType] = field(default_factory=lambda: [TaskType.FRONTEND, TaskType.BACKEND, TaskType.TESTING])
    difficulty_levels: List[DifficultyLevel] = field(default_factory=lambda: [DifficultyLevel.EASY, DifficultyLevel.MEDIUM])
...
class ModelResult:
    ...
    def calculate_summary(self):
        ...
        self.average_execution_time = sum(r.execution_time for r in self.task_results) / self.total_tasks
        
        # Categorize results
        self.frontend_results = [r for r in self.task_results if r.task_type == TaskType.FRONTEND]
        self.backend_results = [r for r in self.task_results if r.task_type == TaskType.BACKEND]
        self.testing_results = [r for r in self.task_results if r.task_type == TaskType.TESTING]
    ...
    def get_leaderboard(self) -> List[Dict[str, Any]]:
        ...
        for model_result in self.model_results:
            leaderboard.append({
                ...
                "pass_rate": model_result.passed_tasks / max(model_result.total_tasks, 1),
                "avg_time": model_result.average_execution_time,
                "frontend_score": sum(r.score for r in model_result.frontend_results) / max(len(model_result.frontend_results), 1),
                "backend_score": sum(r.score for r in model_result.backend_results) / max(len(model_result.backend_results), 1),
                "testing_score": sum(r.score for r in model_result.testing_results) / max(len(model_result.testing_results), 1),
...
class EvaluationEngine:
    ...
    async def run_evaluation(
        ...
        try:
            logger.info(f"Starting evaluation run {run_id} with {len(model_configs)} models")
            ...
            for i, model_config in enumerate(model_configs):
                ...
                try:
                    ...
                    model_result = await self._evaluate_model(model_interface, tasks, config, progress_callback)
            ...
            logger.info(f"Evaluation run {run_id} completed in {evaluation_run.duration:.2f}s")
    ...
    async def _evaluate_model(
        ...
        try:
            ...
            if not model_interface.test_connection():
                raise RuntimeError(f"Cannot connect to model {model_interface.model_name}")
            ...
            if config.include_bigcodebench:
                try:
                    if progress_callback:
                        progress_callback(f"Running BigCodeBench evaluation for {model_interface.model_name}")
                    
                    bigcodebench_results = self.bigcodebench_runner.evaluate_with_bigcodebench(
                        model_interface,
                    ...
                    logger.warning(f"BigCodeBench evaluation failed for {model_interface.model_name}: {e}")
            
            # Calculate summary statistics
            model_result.calculate_summary()
            
        except Exception as e:
            logger.error(f"Model evaluation failed for {model_interface.model_name}: {e}")
        ...
        try:
            ...
            if bigcodebench_results:
                ...
                return TaskResult(
                    ...
                    detailed_scores=bcb_result.test_results.get('details', {}) if bcb_result.test_results else None
                )
            else:
    ...
    def get_evaluation_status(self, run_id: str) -> Optional[Dict[str, Any]]:
        ...
        if run_id in self.active_runs:
            ...
            return {
                ...
                "duration": time.time() - run.timestamp.timestamp() if run.status == "running" else run.duration
            }
        return None
