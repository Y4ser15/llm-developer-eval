...
class EvaluationConfig:
    ...
    domains: List[str] = field(
        default_factory=lambda: ["frontend", "backend", "testing"]
    )
...
class ModelEvaluationResult:
    ...
    coverage_at_k: float = 0.0  # NEW: average coverage ratio
    ...
    def get_leaderboard(self) -> List[Dict[str, Any]]:
        ...
        for result in self.model_results:
            entry = {
                ...
                "coverage_at_k": result.coverage_at_k,  # NEW
...
class ComprehensiveEvaluator:
    ...
    async def evaluate_models(
        self, model_configs: List[ModelConfig], config: EvaluationConfig = None
        ...
        try:
            logger.info(
                f"🚀 Starting comprehensive evaluation {run_id} with {len(model_configs)} models"
            )
            ...
            logger.info(
                f"✅ Evaluation {run_id} completed in {evaluation_run.total_duration:.2f}s"
            )
    ...
    async def _evaluate_models_sequential(
        self, model_configs: List[ModelConfig], config: EvaluationConfig
    ...
    async def _evaluate_single_model(
        self, model_config: ModelConfig, config: EvaluationConfig
        ...
        try:
            ...
            logger.info(
                f"✅ Completed evaluation for {model_config.name}: {result.passed_tasks}/{result.total_tasks} passed"
            )
    ...
    def _calculate_summary_metrics(self, result: ModelEvaluationResult):
        ...
        # NEW: Compute average coverage@k
        coverages = [
            r.test_results.get("coverage_at_k", 0.0)
            for r in all_results
            if hasattr(r, "test_results") and r.test_results
        ]
        result.coverage_at_k = sum(coverages) / len(coverages) if coverages else 0.0

        # Domain-specific scores
        for domain_name, domain_results in result.domain_results.items():
            if domain_results:
                domain_score = sum(r.score for r in domain_results) / len(
                    domain_results
                )
    ...
    async def _generate_comprehensive_report(
        self, evaluation_run: ComprehensiveEvaluationRun
    ):
        """Generate comprehensive HTML report"""

        try:
            report_path = (
                self.results_dir / f"comprehensive_report_{evaluation_run.run_id}.html"
            )
    ...
    def _save_evaluation_results(
        self, evaluation_run: ComprehensiveEvaluationRun, format: str = "json"
    ):
        """Save evaluation results in specified format"""

        try:
            base_path = (
                self.results_dir / f"comprehensive_results_{evaluation_run.run_id}"
            )
            ...
            for model_result in evaluation_run.model_results:
                model_data = {
                    ...
                    "summary": {
                        ...
                        "coverage_at_k": model_result.coverage_at_k,  # NEW
